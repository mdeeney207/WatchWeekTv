 ...
 *
 * Notes:
 * - runtime now comes directly from v_week_episodes_all (no second pass)
 */
...
export default function WeekScreen() {
  ...
  // ✅ RefreshBus does NOT provide token; subscribe to emits instead.
  const { bump, subscribe } = useRefreshBus();
  ...
  useEffect(() => {
    ...
    if (!keys.has(selectedDayKey)) setSelectedDayKey(keys.has(tKey) ? tKey : "ALL");
  ...
  const load = useCallback(async () => {
    ...
    if (!uid) {
      // i18n
      setErrorText(t("errors.notSignedIn"));
      setRows([]);
      return;
    }
    ...
    const serviceIds = Array.from(
      new Set(base.map((r: any) => r.service_id).filter(Boolean))
    ) as string[];
    ...
    const enriched: Row[] = base.map((r: any) => {
  ...
  const timeForecast = useMemo(() => {
    ...
    for (const r of visibleRows as any[]) {
      ...
      const name = r.service_name ?? t("common.unknown");
  ...
  useEffect(() => {
    ...
    const tKey = todayKeyLocal();
    const idx = days.findIndex((d) => dateKeyLocal(d) === tKey);
    if (idx < 0) return;
  ...
  async function handleOpen(r: Row) {
    try {
      ...
      const ok = typeof outcome === "boolean" ? outcome : !!(outcome as any)?.ok;

      if (!ok) {
        Alert.alert(t("week.noLinkTitle"), t("week.noLinkBody"));
      }
    } catch (e: any) {
      Alert.alert(t("week.openFailedTitle"), e?.message ?? t("errors.generic"));
    } finally {
  ...
  async function handleWatched(r: Row) {
    try {
      ...
      bump(); // emits refresh bus (legacy-safe)
    } catch (e: any) {
      ...
      Alert.alert(t("week.couldNotMarkWatchedTitle"), e?.message ?? t("errors.generic"));
    } finally {
      setBusyEpisodeId(null);
    }
  }
      ...
      <View style={{ paddingHorizontal: THEME.pagePad ?? 18, paddingTop: 14, paddingBottom: 6 }}>
        <Section>
            ...
            >
              ...
              {timeForecast.missingRuntimeCount > 0 && (
                <Text style={{ color: THEME.textMuted, marginTop: 4, fontSize: 12 }}>
                  {t("week.timeForecastMissing", { count: timeForecast.missingRuntimeCount })}
                </Text>
              )}
                ...
                <View style={{ marginTop: 10, gap: 6 }}>
                  ...
                  {timeForecast.services.length > 5 && (
                    <Text style={{ color: THEME.textMuted, marginTop: 2, fontSize: 12 }}>
                      {t("week.moreServices", { count: timeForecast.services.length - 5 })}
                    </Text>
                  )}
                </View>
              )}
          ...
          >
            ...
            >
              <Text style={{ fontWeight: "900", color: THEME.text }}>{t("common.all")}</Text>
            ...
            {days.map((d) => {
              ...
              const weekday = d.toLocaleDateString(i18n.language || undefined, { weekday: "short" });
              ...
              return (
                <Pressable
                  ...
                  style={[chipBase, { borderColor, backgroundColor: bg, opacity: isSelected ? 1 : 0.94 }]}
                >
  ...
  }, [visibleCount, timeForecast, days, countsByDay, selectedDayKey, selectedIsAll, todayKey, t, i18n.language]);

  const sections: WeekSection[] = useMemo(() => {
    const buildTitle = (d: Date) =>
      d.toLocaleDateString(i18n.language || undefined, { weekday: "long", month: "short", day: "numeric" });
      ...
      return [
        {
          key: selectedDayKey,
          title,
          subtitle: items.length ? undefined : t("week.noReleases"),
          data: items,
        },
      ];
    }
  ...
  }, [selectedDayKey, grouped, days, visibleCount, t, i18n.language]);
  ...
  return (
    <>
        ...
        <View style={{ flex: 1, backgroundColor: THEME.bgSolid }}>
          ...
          {visibleCount === 0 ? (
            <View style={{ paddingHorizontal: THEME.pagePad ?? 18, paddingTop: 14 }}>
              <EmptyState title={t("week.emptyTitle")} subtitle={t("week.emptySubtitle")} />
            </View>
          ) : (
            <SectionList
              sections={sections}
              ...
              renderSectionHeader={({ section }) => (
                ...
                >
                  <View style={{ flexDirection: "row", alignItems: "baseline", justifyContent: "space-between" }}>
                    ...
                    <Text style={{ color: THEME.textMuted, fontWeight: "900", fontSize: 12 }}>{section.data.length}</Text>
                  </View>
                  {!!section.subtitle && (
              ...
              ListEmptyComponent={
                <View style={{ paddingHorizontal: THEME.pagePad ?? 18, paddingTop: 14 }}>
                  <EmptyState title={t("week.noReleases")} subtitle={t("week.pickAnotherDay")} />
                </View>
              }
            />
          )}
      ...
      <Toast
        ...
        message={t("week.toastMarkedWatched")}
